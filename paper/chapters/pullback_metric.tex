...
Throughout this chapter let $F:\mathcal P \longrightarrow F(\mathcal P)$ denote the logarithmic quotient map, which is a local diffeomorphism by Theorem~\ref{ch:thm:5-4} of Chapter~\ref{ch:sec:local-diffeomorphism}.
The image \(F(\mathcal P)\) is an open subset of the finite-dimensional vector space
\[
\prod_{c\in\mathcal C} T_{P_c}\Delta_c^\circ \cong \mathbb R^{n-|\mathcal C|},
\]
where each factor \(T_{P_c}\Delta_c^\circ=\{v:\sum_{i\in c}v_i=0\}\) is the
(constant, tangent) subspace of mean-zero vectors on the cloak \(c\).
...
The pullback metric is the unique Riemannian structure that makes the logarithmic map a local isometry. This metric defines geodesic distances on the posterior manifold, which in turn determine the Lipschitz constants for information functionals (Chapters~\ref{ch:sec:entropy-bounds}--\ref{ch:sec:the-average-hessian-operator-and-universal-bregman-theo}) and the variational structure of the UOD (Chapters~\ref{ch:sec:analytic-uod}--\ref{ch:sec:variational-uod}).
...
The image $F(\mathcal P)$ is a subset of Euclidean space, so it inherits the Euclidean metric.  But what we need is a metric on the posterior manifold itself---a way to measure how much two distributions differ in terms of their cryptographic structure.  The pullback metric $g=F^*g_E$ is the unique Riemannian metric that makes $F$ a local isometry: infinitesimal distances on the manifold are the Euclidean distances between the images of tangent vectors, and geodesic distance on the manifold is computed by integrating these infinitesimal lengths.  This identification is the bridge between geometry (distributions) and algebra (WIS factorisation).
...
\end{proposition}
\begin{proof}
By Theorem~\ref{ch:thm:5-4}, \(F\) is a local diffeomorphism, so in a neighbourhood of every point the differential

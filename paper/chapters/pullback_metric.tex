...
\end{proof}
%% Explanatory comment: The Euclidean metric on the image space is smooth, which is essential because pulling back a smooth metric by a diffeomorphism yields a smooth Riemannian metric on the posterior manifold. This ensures that all subsequent geometric computations are well-defined.
...
\end{proof}
%% Explanatory comment: The pullback metric is well-defined because F is a diffeomorphism: the differential DF_P is invertible, so the Euclidean metric on the image is transported uniquely back to the tangent space of the posterior manifold. Without this invertibility, the pullback would not be uniquely defined.
...
\]
\end{proof}
%% Explanatory comment: Symmetry of the pullback metric follows directly from the symmetry of the Euclidean inner product. This property is necessary for the pullback to qualify as a genuine Riemannian metric, as Riemannian metrics are always symmetric bilinear forms.
...
\end{proof}
%% Explanatory comment: The pullback metric is positive definite because the differential DF_P is invertible and the Euclidean metric is positive definite. This means that the induced norm is a genuine measure of length on the posterior manifold: non-zero tangent vectors have positive length.
...
\end{proof}
%% Explanatory comment: The pullback metric defines a smooth Riemannian metric on the posterior manifold, equipping it with a genuine geometric structure. This is the foundational result for all subsequent geometric analysis: distances, angles, gradients, and curvatures on the posterior manifold are now well-defined through this metric.
...
\end{proof}
%% Explanatory comment: The matrix representation J^T J of the pullback metric is positive definite at every point, confirming that the metric is non-degenerate. This representation also shows that the metric can be expressed in terms of the Jacobian of the logarithmic map, making it computable in practice.
...
For differential privacy (Section~\ref{ch:sec:differential-privacy-and-database-queries}), the pullback metric $g=F^*g_E$ measures the cost of moving between neighbouring output distributions.  For the Laplace mechanism with $\mathcal D=2/\varepsilon^2$, the pullback metric at $P$ is $g_P(u,v)=F^*(u)^T F^*(v)/\|F(P)\circ F(Q)\|^2$, and near the uniform prior $U_n$ this gives $g_P(u,v)\approx (\varepsilon^2/2)\langle u,v\rangle$---a direct link between the privacy budget and the geometry of the posterior manifold.

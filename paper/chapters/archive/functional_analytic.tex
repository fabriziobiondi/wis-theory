\chapter[Functional-Analytic Properties]{Functional-Analytic Properties of the Universal Optimality Defect}
...
probability simplex. Part III proves that \(F\) is a global
...
\section{Positivity and Separation}
...
\(\mathcal D(P,Q)=0\), then \(F(P)=F(Q)\). By the results of Part III, \(F\) is
...
\section{Local Lipschitz Continuity}
...
\section{An Auxiliary Linear Lemma}
...
\(\mathcal P^\circ\).
\end{theorem}
\begin{proof} Part III proves that \(F\) is a homeomorphism onto its image.

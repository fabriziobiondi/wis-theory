...
the normalised ratio $n_{mc}=w(m)/\beta(c)$ that factors out the key
set size.
...
\end{definition}
\begin{proof}
For a fixed message $m$, each key $k\in\mathcal K$ produces exactly one
ciphertext $c=E(m,k)$.  Hence the sum over $c$ of the number of keys
mapping $m$ to $c$ is precisely $|\mathcal K|$.
\end{proof}

...
\]
denote the canonical logarithmic coordinate map introduced in Chapter~\ref{ch:sec:the-posterior-manifold}.
...
\end{proof}
%% Explanatory comment: The canonical logarithmic map F is smooth because it is composed of smooth functions. This smoothness is the minimal regularity needed to apply the tools of differential geometry, including the Inverse Function Theorem used later in this chapter.
...
\end{proof}
%% Explanatory comment: The differential of F at any point is simply the composition of the logarithmic differential with the quotient projection. This explicit expression allows the computation of the differential in coordinates, which is essential for the metric constructions in the following chapters.
...
\end{proof}
%% Explanatory comment: The differential DF_P is invertible at every point, which means that F is an immersion and a submersion simultaneously. This is the key analytical condition that, by the Inverse Function Theorem, makes F a local diffeomorphism.
...
\end{proof}
%% Explanatory comment: The canonical logarithmic map F is a local diffeomorphism everywhere on the posterior simplex. This means that, locally, the geometry of the simplex is isomorphic to the geometry of the linear quotient space. Consequently, all local differential-geometric constructions (metrics, connections, geodesics) can be transported from the quotient space to the simplex.
...
\end{proof}
%% Explanatory comment: The logarithmic quotient coordinates provide a valid local chart on the posterior manifold. This means that every smooth geometric quantity — functions, vector fields, tensors — can be expressed locally in these coordinates, which is essential for the explicit metric computations carried out in subsequent chapters.

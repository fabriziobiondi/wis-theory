...
The stability results of this chapter---continuity, local quadratic approximation, strict convexity of the UOD---are the foundation for the quantitative bounds that follow. The Lipschitz and quadratic estimates of Chapters~\ref{ch:sec:information-functionals-on-the-wis-manifold}--\ref{ch:sec:the-average-hessian-operator-and-universal-bregman-theo} all rely on the quadratic approximation established here.
...
Writing \(\Pi\) for the orthogonal projector onto
\(\operatorname{Im}(C)^\perp\) introduced in
Chapter~\ref{ch:sec:hilbert-geometry} (so that
\(\mathcal D(Y)=\|\Pi Y\|^2\) by the characterisation of
Chapter~\ref{ch:sec:hilbert-geometry}), the defect is
\[
\mathcal D(P)
=
\|\Pi\,(F(P)-F(P^\star))\|^2
...
Writing $\eta(P)=F(P)-F(P^\star)$ for the coordinate displacement, the definition of the defect gives
\[
\mathcal D(P)
=
g_{P^\star}(\eta(P),\eta(P)).
\]
(Here the projector \(\Pi\) acts as the identity on the tangent
space of the quotient, so \(\|\Pi\eta\|^2=\|\eta\|^2\); the
displayed formula is the same as
\(\|\Pi\,(F(P)-F(P^\star))\|^2\) of Proposition~\ref{ch:prop:9-1}.)
...
provided the coordinate map \(F\) has full-rank differential at \(P^\star\)
(equivalently, the UOD is not identically zero, as in the degenerate
AES case discussed below).
\end{theorem}
\begin{proof}
Since $P^\star$ is the reference posterior, $\mathcal D(P^\star)=0$ by definition.
The defect is a squared norm, so $\mathcal D(P)\ge0$

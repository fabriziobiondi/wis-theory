...
is continuous. 
\end{proof}
%% Explanatory comment: Continuity of the UOD guarantees that the functional does not exhibit jumps or discontinuities under small perturbations of the posterior distribution. This is the minimal regularity required for any meaningful optimization or sensitivity analysis.
...
\end{proof}
%% Explanatory comment: The UOD behaves quadratically near the reference posterior, with no linear term in the expansion. This means that the optimality defect is a second-order quantity: small deviations from the optimal posterior produce deviations in the UOD that scale as the square of the perturbation size.
...
\end{proof}
%% Explanatory comment: The reference posterior is a strict local minimizer of the UOD, meaning that any sufficiently small nonzero perturbation increases the defect value. This establishes the posterior as a locally unique optimum, confirming that the UOD is a reliable measure of deviation.

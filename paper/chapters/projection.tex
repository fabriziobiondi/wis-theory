...
Recall the quotient space (Section~\ref{ch:sec:canonical-projection-and-differential-factorisation})
\[
\mathcal G
=
\mathbb R^k
...
is injective. 
\end{proof}
%% Explanatory comment: Injectivity means that no non-zero vector in the weighted hyperplane W_P projects to the zero element in the quotient. Consequently, the gauge symmetry has been completely eliminated: the only direction that could collapse to zero is the constant vector, which is already removed from W_P.
...
is surjective. 
\end{proof}
%% Explanatory comment: Surjectivity combined with injectivity shows that the restricted projection is a bijection. This means that every element of the quotient space can be represented uniquely by a vector in the weighted hyperplane, establishing a canonical identification between the two spaces.
...
\end{proof}
%% Explanatory comment: The weighted hyperplane W_P and the quotient space are linearly isomorphic. This means that the weighted hyperplane provides an explicit model for the quotient, allowing concrete computations while retaining the gauge-invariant geometric structure.
...
\end{proof}
%% Explanatory comment: The Fundamental Differential Factorization shows that the differential of the canonical logarithmic map factors into two linear isomorphisms: the logarithmic differential DL_P followed by the quotient projection Q. This factorisation is the central structural result: it reduces the geometry of the posterior simplex to the simpler linear algebra of the quotient space.
...
\end{proof}
%% Explanatory comment: The differential of the canonical logarithmic map F = Q∘L is invertible at every point of the posterior simplex. This invertibility is the key analytical fact that enables the Inverse Function Theorem, which in turn implies that F is a local diffeomorphism — the subject of the next chapter.

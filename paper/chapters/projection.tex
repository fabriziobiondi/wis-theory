...
In the AES example (Section~\ref{ch:sec:aes-and-perfect-secrecy}), the posterior manifold is the set of all output distributions over $\mathcal M=\mathcal C=\mathbb F_2^{128}$.  The logarithmic map $L(P)=\log P$ sends the simplex to a hyperplane in $\mathbb R^n$, and the differential $DL_P$ gives the linearised effect of a key perturbation.  For a fixed ciphertext $c$ the posterior is a single message (the cloak is trivial), so $\dim(\operatorname{Im}(DL_P))=k-1$ by Corollary~\ref{ch:cor:3-3}---every posterior variation is captured by a unique logarithmic direction, and the AES factorisation $n_{mc}=1$ makes the defect identically zero.

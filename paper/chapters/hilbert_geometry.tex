...
The Hilbert-space geometry of this chapter gives the first quantitative measure of deviation from universal optimality. The posterior Laplacian and the quotient geometry of Chapters~\ref{ch:sec:the-posterior-manifold}--\ref{ch:sec:pullback-riemannian-metric} are direct consequences of the least-squares formulation introduced here.
...
\end{equation}
The quadratic part $L=A^{\mathsf T}\Pi A$ is the \emph{posterior Laplacian}.  Expanding this expression explicitly,
\[
...
Its nullspace satisfies $\ker L = \{u\in V_M : \Pi A u = 0\},$ which consists precisely of the message potentials that can be compensated by a ciphertext potential without changing the energy.
\end{lemma}
\begin{proof}
...
The Universal Optimality Defect $\mathcal D(Y)=\|q(Y)\|^2=\|\Pi Y\|^2$ is the squared distance from the incidence vector $Y$ to the subspace $\operatorname{Im}(C)$ of universally optimal encoders.
...
All structures (incidence spaces, lifting operators, the quotient map,
and the gauge quotient) remain well defined; only the gauge class is non-zero.  By

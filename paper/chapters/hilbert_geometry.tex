...
$DE_\omega(x)[h]=2\langle\Phi x-\omega,\Phi hangle
=2\langle\Phi^{\mathsf T}(\Phi x-\omega),hangle$.
\end{proof}
...
$Au-Cv^\star-\omega = Q(Au-\omega),$
where $Q=I-C D_C^{-1}C^{\mathsf T}$ is the orthogonal projector onto $\operatorname{Im}(C)^\perp$.
...
E_{\mathrm{red}}(u)=u^{\mathsf T}A^{\mathsf T}QA\,u
-2u^{\mathsf T}A^{\mathsf T}Q\omega
+\omega^{\mathsf T}Q\omega.
\end{equation}
The quadratic part $L=A^{\mathsf T}QA$ is the \emph{posterior Laplacian}.  Writing $Q$ explicitly,
\[
L = A^{\mathsf T}QA = D_M - H D_C^{-1} H^{\mathsf T}
...
\end{theorem}
\begin{proof}
The Hessian is $\nabla^2 E_{\mathrm{red}}=2L=2A^{\mathsf T}QA$.
Since $Q^2=Q$ and $Q^{\mathsf T}=Q$,
\[
x^{\mathsf T}Lx = x^{\mathsf T}A^{\mathsf T}QAx = (Ax)^{\mathsf T}Q(Ax)
= \|Q(Ax)\|^2 \ge 0
\]
...
$x^{\mathsf T}Lx = \|Q(Ax)\|^2 \ge 0$, so $L\succeq 0$.
Moreover, $Lx=0$ iff $\|Q(Ax)\|^2=0$, i.e.\ $QAx=0$.  By definition
of $Q$, this means $Ax\in\operatorname{Im}(C)$, which is exactly the
...
\]
The quotient space $V_I/\operatorname{Im}(C)$ is the primary geometric object.  It is determined solely by the posterior symmetry of Bayes' rule, independently of any metric or variational principle.  The orthogonal projector $Q$ selects the unique minimum-norm representative in each equivalence class.
...
The Universal Optimality Defect $\mathcal D(P)=\|\delta(P)\|^2$ is the squared distance from a distribution to the set of universally optimal ones.
...
\item Eliminating ciphertext potentials yields the reduced energy $E_{\mathrm{red}}$ and the posterior Laplacian $L=A^{\mathsf T}QA$.

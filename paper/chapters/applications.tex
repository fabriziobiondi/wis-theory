...
logarithmic factorisation $\log n_{mc}=u_m+v_c$ with
...
\paragraph{UOD analysis.}
The WIS factorisation $\log n_{mc}\approx u_m+v_c$ reveals which
...
\paragraph{Example.}
Consider a mix network with $n=100$ messages, $r=3\) mix nodes,
and $\alpha=0.2$ (one compromised node).
Then $\mathcal D\approx0.05\) bits, and the bound gives
$H(P_m) \ge \log_2 100 - 0.025 \approx 6.57$ bits.
The anonymity set size is $2^{6.57}\approx95$---close to the
ideal $100$.
...
$\log_2 T \ge 8 - 3.5/(2\cdot0.004) \approx 8 - 437 = \text{vacuous}$.
...
bits for two keys $m,m'$, then distinguishing them requires at
least $T\ge10$ traces.
...
\paragraph{UOD analysis.}
The WIS factorisation $n_{mc}=e^{u_m+v_c}$ gives

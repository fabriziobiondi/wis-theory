...
\end{theorem}
\begin{proof}
On a regularity interval every comparison has a fixed outcome.
Consequently all maxima, minima and ordering operators reduce to fixed
index selections. Under assumption (2), the resulting expression is
$P_i(t)\propto P_i^{1-t}Q_i^{t}.$. Therefore $P_i(t)=P_j(t).$ is $\log(P_i/P_j)\neq\log(Q_i/Q_j),$ throughout the interval.
\end{proof}

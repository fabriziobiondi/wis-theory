...
$|\mathrm{Cloak}(c)| = 1/\Pr[M=m\mid C=c]$
for every $m\in\mathrm{Cloak}(c)$ (the posterior is uniform on the
cloak, by Proposition~\ref{ch:prop:factorisation-criterion})
...
described in Section~\ref{ch:sec:discussion-what-the-three-level-structure-implies}
should mark qualitative changes in the
flow landscape.
\item \textbf{Finite-dimensional reduction.}  Can the three-level
structure be exploited for numerical computation in high dimensions?
\end{itemize}

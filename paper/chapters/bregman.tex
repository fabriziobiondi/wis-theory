...
The Average Hessian Operator of this chapter turns the universal inequalities of Chapter~\ref{ch:sec:universal-geometric-bounds} into exact equalities for Bregman divergences. This operator plays a central role in the variational theory of Part~V, where it connects the UOD to the spectral geometry of the posterior Laplacian.
...
\begin{lemma}[Local Geodesic Convexity of the KL Divergence]
\label{ch:lem:kl-convex}
For every fixed $Q$, the Kullback--Leibler divergence $P\mapsto D_{\mathrm{KL}}(P\|Q)$ is
geodesically convex on every neighbourhood of $Q$ in the posterior manifold.
\end{lemma}
...
Each KL divergence is locally geodesically convex in a neighbourhood
of its reference (Lemma~\ref{ch:lem:kl-convex}), and the average preserves
convexity.  Therefore JSD is locally geodesically
